...
void DlgProductScore::read_config()
{
	...
	// 撕裂
	ui->rbtn_silieEnable->setChecked(scoreConfig.siLie);
	ui->pbtn_silieSimilarity->setText(QString::number(scoreConfig.siLieScore));
	ui->pbtn_silieArea->setText(QString::number(scoreConfig.siLieArea));

	// 缺失
	ui->rbtn_queshiEnable->setChecked(scoreConfig.queShi);
	ui->pbtn_queshiSimilarity->setText(QString::number(scoreConfig.queShiScore));
	ui->pbtn_queshiArea->setText(QString::number(scoreConfig.queShiArea));
...
void DlgProductScore::build_connect()
{
	...
	QObject::connect(ui->rbtn_silieEnable, &QRadioButton::clicked,
		this, &DlgProductScore::rbtn_silieEnable_checked);
	QObject::connect(ui->pbtn_silieSimilarity, &QPushButton::clicked,
		this, &DlgProductScore::pbtn_silieSimilarity_clicked);
	QObject::connect(ui->pbtn_silieArea, &QPushButton::clicked,
		this, &DlgProductScore::pbtn_silieArea_clicked);
	QObject::connect(ui->rbtn_queshiEnable, &QRadioButton::clicked,
		this, &DlgProductScore::rbtn_queshiEnable_checked);
	QObject::connect(ui->pbtn_queshiSimilarity, &QPushButton::clicked,
		this, &DlgProductScore::pbtn_queshiSimilarity_clicked);
	QObject::connect(ui->pbtn_queshiArea, &QPushButton::clicked,
		this, &DlgProductScore::pbtn_queshiArea_clicked);
...
void DlgProductScore::rbtn_silieEnable_checked()
{
	auto& scoreConfig = Modules::getInstance().configManagerModule.scoreConfig;
	scoreConfig.siLie = ui->rbtn_silieEnable->isChecked();
}

void DlgProductScore::pbtn_silieSimilarity_clicked()
{
	NumberKeyboard numKeyBord;
	numKeyBord.setWindowFlags(Qt::Window | Qt::CustomizeWindowHint);
	auto isAccept = numKeyBord.exec();
	if (isAccept == QDialog::Accepted)
	{
		...
		auto& scoreConfig = Modules::getInstance().configManagerModule.scoreConfig;
		ui->pbtn_silieSimilarity->setText(value);
		scoreConfig.siLieScore = value.toDouble();
	}
}

void DlgProductScore::pbtn_silieArea_clicked()
{
	NumberKeyboard numKeyBord;
	numKeyBord.setWindowFlags(Qt::Window | Qt::CustomizeWindowHint);
	auto isAccept = numKeyBord.exec();
	if (isAccept == QDialog::Accepted)
	{
		...
		auto& scoreConfig = Modules::getInstance().configManagerModule.scoreConfig;
		ui->pbtn_silieArea->setText(value);
		scoreConfig.siLieArea = value.toDouble();
	}
}

void DlgProductScore::rbtn_queshiEnable_checked()
{
	auto& scoreConfig = Modules::getInstance().configManagerModule.scoreConfig;
	scoreConfig.queShi = ui->rbtn_queshiEnable->isChecked();
}

void DlgProductScore::pbtn_queshiSimilarity_clicked()
{
	NumberKeyboard numKeyBord;
	numKeyBord.setWindowFlags(Qt::Window | Qt::CustomizeWindowHint);
	auto isAccept = numKeyBord.exec();
	if (isAccept == QDialog::Accepted)
	{
		...
		auto& scoreConfig = Modules::getInstance().configManagerModule.scoreConfig;
		ui->pbtn_queshiSimilarity->setText(value);
		scoreConfig.queShiScore = value.toDouble();
	}
}

void DlgProductScore::pbtn_queshiArea_clicked()
{
	NumberKeyboard numKeyBord;
	numKeyBord.setWindowFlags(Qt::Window | Qt::CustomizeWindowHint);
	auto isAccept = numKeyBord.exec();
	if (isAccept == QDialog::Accepted)
	{
		...
		auto& scoreConfig = Modules::getInstance().configManagerModule.scoreConfig;
		ui->pbtn_queshiArea->setText(value);
		scoreConfig.queShiArea = value.toDouble();
	}
}

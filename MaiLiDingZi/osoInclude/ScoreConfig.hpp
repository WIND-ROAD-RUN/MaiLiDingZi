...
namespace cdm {
    ...
    inline ScoreConfig::ScoreConfig(const rw::oso::ObjectStoreAssembly& assembly)
    {
        ...
        auto siLieItem = rw::oso::ObjectStoreCoreToItem(assembly.getItem("$variable$siLie$"));
        if (!siLieItem) {
            throw std::runtime_error("$variable$siLie is not found");
        }
        siLie = siLieItem->getValueAsBool();
        auto siLieScoreItem = rw::oso::ObjectStoreCoreToItem(assembly.getItem("$variable$siLieScore$"));
        if (!siLieScoreItem) {
            throw std::runtime_error("$variable$siLieScore is not found");
        }
        siLieScore = siLieScoreItem->getValueAsDouble();
        auto siLieAreaItem = rw::oso::ObjectStoreCoreToItem(assembly.getItem("$variable$siLieArea$"));
        if (!siLieAreaItem) {
            throw std::runtime_error("$variable$siLieArea is not found");
        }
        siLieArea = siLieAreaItem->getValueAsDouble();
        auto queShiItem = rw::oso::ObjectStoreCoreToItem(assembly.getItem("$variable$queShi$"));
        if (!queShiItem) {
            throw std::runtime_error("$variable$queShi is not found");
        }
        queShi = queShiItem->getValueAsBool();
        auto queShiScoreItem = rw::oso::ObjectStoreCoreToItem(assembly.getItem("$variable$queShiScore$"));
        if (!queShiScoreItem) {
            throw std::runtime_error("$variable$queShiScore is not found");
        }
        queShiScore = queShiScoreItem->getValueAsDouble();
        auto queShiAreaItem = rw::oso::ObjectStoreCoreToItem(assembly.getItem("$variable$queShiArea$"));
        if (!queShiAreaItem) {
            throw std::runtime_error("$variable$queShiArea is not found");
        }
        queShiArea = queShiAreaItem->getValueAsDouble();
    ...
    inline ScoreConfig::ScoreConfig(const ScoreConfig& obj)
    {
        ...
        siLie = obj.siLie;
        siLieScore = obj.siLieScore;
        siLieArea = obj.siLieArea;
        queShi = obj.queShi;
        queShiScore = obj.queShiScore;
        queShiArea = obj.queShiArea;
    ...
    inline ScoreConfig::operator rw::oso::ObjectStoreAssembly() const
    {
        ...
        auto siLieItem = std::make_shared<rw::oso::ObjectStoreItem>();
        siLieItem->setName("$variable$siLie$");
        siLieItem->setValueFromBool(siLie);
        assembly.addItem(siLieItem);
        auto siLieScoreItem = std::make_shared<rw::oso::ObjectStoreItem>();
        siLieScoreItem->setName("$variable$siLieScore$");
        siLieScoreItem->setValueFromDouble(siLieScore);
        assembly.addItem(siLieScoreItem);
        auto siLieAreaItem = std::make_shared<rw::oso::ObjectStoreItem>();
        siLieAreaItem->setName("$variable$siLieArea$");
        siLieAreaItem->setValueFromDouble(siLieArea);
        assembly.addItem(siLieAreaItem);
        auto queShiItem = std::make_shared<rw::oso::ObjectStoreItem>();
        queShiItem->setName("$variable$queShi$");
        queShiItem->setValueFromBool(queShi);
        assembly.addItem(queShiItem);
        auto queShiScoreItem = std::make_shared<rw::oso::ObjectStoreItem>();
        queShiScoreItem->setName("$variable$queShiScore$");
        queShiScoreItem->setValueFromDouble(queShiScore);
        assembly.addItem(queShiScoreItem);
        auto queShiAreaItem = std::make_shared<rw::oso::ObjectStoreItem>();
        queShiAreaItem->setName("$variable$queShiArea$");
        queShiAreaItem->setValueFromDouble(queShiArea);
        assembly.addItem(queShiAreaItem);
    ...
    {
        return zangWu == obj.zangWu && zangWuScore == obj.zangWuScore && zangWuArea == obj.zangWuArea && jieTou == obj.jieTou && jieTouScore == obj.jieTouScore && jieTouArea == obj.jieTouArea && xiaoZangWu == obj.xiaoZangWu && xiaoZangWuScore == obj.xiaoZangWuScore && xiaoZangWuArea == obj.xiaoZangWuArea && body == obj.body && bodyScore == obj.bodyScore && bodyArea == obj.bodyArea && huaPo == obj.huaPo && huaPoScore == obj.huaPoScore && huaPoArea == obj.huaPoArea && siLie == obj.siLie && siLieScore == obj.siLieScore && siLieArea == obj.siLieArea && queShi == obj.queShi && queShiScore == obj.queShiScore && queShiArea == obj.queShiArea && weiZhiQueXianSingle == obj.weiZhiQueXianSingle && weiZhiQueXianSingleArea == obj.weiZhiQueXianSingleArea && weiZhiQueXianAll == obj.weiZhiQueXianAll && weiZhiQueXianAllArea == obj.weiZhiQueXianAllArea;
    }
